...
\begin{frame}{Punto de partida}
  ...
  \begin{itemize}
    ...
    \item búsqueda en anchura (\emph{Breadth-First Search}, BFS), búsqueda en profundidad (\emph{Depth-First Search}, DFS) y búsqueda de costo uniforme (\emph{Uniform-Cost Search}, UCS);
...
\begin{frame}{Una familia, distintas prioridades}
  \small
  \begin{tabularx}{\textwidth}{>{\bfseries\raggedright\arraybackslash}p{.14\textwidth}>{\raggedright\arraybackslash}p{.27\textwidth}X}
    \toprule
    Estrategia & Prioridad & Pregunta que privilegia \\\midrule
    BFS & profundidad & ¿Cuántos pasos llevo? \\
    DFS & LIFO; mayor profundidad & ¿Qué rama continúo primero? \\
  ...
  \begin{block}{Siglas}
    \scriptsize
    \textbf{BFS}: \emph{Breadth-First Search} (búsqueda en anchura).\\
    \textbf{DFS}: \emph{Depth-First Search} (búsqueda en profundidad).\\
    \textbf{UCS}: \emph{Uniform-Cost Search} (búsqueda de costo uniforme).
  \end{block}
\end{frame}
...
% 12
\begin{frame}{Cómo interpretar $g(n)$, $h(n)$ y $f(n)$}
  \small
  Para $C$, alcanzado mediante $S\rightarrow A\rightarrow C$:
  \begin{table}
  \begin{tabularx}{\textwidth}{>{\bfseries}p{.15\textwidth}p{.18\textwidth}X}
    \toprule
    Función & Cálculo & Interpretación \\\midrule
  ...
  \end{table}
  \begin{itemize}
    \item $g$ depende del camino; $h$, del estado y del objetivo.
    \item $f$ evalúa prioridad: no es un costo ya pagado.
    \item UCS prioriza $g$; voraz, $h$; A*, $f=g+h$.
  \end{itemize}
  \centering\concepto{$g$ resume el pasado, $h$ estima el futuro y $f$ combina ambos.}
\end{frame}
...
% 29
\begin{frame}{Ejemplo de heurística admisible}
  \small
  En el grafo didáctico podemos calcular el costo óptimo restante $h^*(n)$:
  \begin{table}
  \begin{tabular}{crrc}
    \toprule
    Estado & $h(n)$ & $h^*(n)$ & Comprobación \\\midrule
    $S$ & 7 & 7 & $7\leq7$ \\
    $A$ & 5 & 5 & $5\leq5$ \\
    $B$ & 7 & 8 & $7\leq8$ \\
    $C$ & 3 & 3 & $3\leq3$ \\
  ...
  \end{tabular}
  \end{table}
  La heurística puede subestimar y seguir siendo admisible; no necesita coincidir con $h^*$.
  \vfill
...
\begin{frame}{Intuición de optimalidad}
  ...
  \]
  En una meta, $h(G)=0$. Para una solución subóptima de costo $C>C^*$:
  \[
    f(G)=g(G)=C>C^*
  \]
  \begin{block}{Implicancia}
    Mientras la frontera conserve un nodo del camino óptimo con $f(n)\leq C^*$, A* lo extraerá antes que una meta subóptima con $f(G)>C^*$.
  \end{block}
  \begin{exampleblock}{Ejemplo}
    Si $C^*=7$ y aparece una meta de costo 9, un candidato óptimo con $f\leq7$ tiene prioridad sobre esa meta.
  \end{exampleblock}
  \scriptsize Esta conclusión requiere comprobar la meta al extraerla y respetar las demás condiciones de A*.
\end{frame}
...
\begin{frame}{Comparación de estrategias}
  \small
  \begin{tabularx}{\textwidth}{>{\bfseries\raggedright\arraybackslash}p{.14\textwidth}>{\raggedright\arraybackslash}p{.25\textwidth}>{\raggedright\arraybackslash}p{.18\textwidth}X}
    \toprule
