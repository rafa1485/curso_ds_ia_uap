...
\begin{frame}{Propósito de la clase}
  ...
  \begin{block}{Al finalizar, podremos}
    \begin{itemize}
      ...
      \item explicar KDD (\textit{Knowledge Discovery in Databases}) y CRISP-DM (\textit{Cross-Industry Standard Process for Data Mining});
...
\begin{frame}{KDD: del dato al conocimiento validado}
  \centering
  \small\textbf{KDD}: \textit{Knowledge Discovery in Databases}\\
  \textcolor{UAPGray}{Descubrimiento de Conocimiento en Bases de Datos}
  \vspace{0.35em}
  \centering
  \begin{tikzpicture}[node distance=4mm,scale=0.80,transform shape]
...
% 22
\begin{frame}{Etapas de KDD explicadas}
  \scriptsize
  \begin{tabularx}{\textwidth}{>{\bfseries}p{0.18\textwidth} X X}
    \toprule
    ...
    Preprocesamiento & Examinar faltantes, errores, duplicados, sesgos y consistencia & Calidad conocida y decisiones registradas \\
    Transformación & Agregar, codificar, escalar o construir atributos & Representación adecuada para el análisis \\
    Minería de datos & Encontrar patrones o ajustar modelos & Resultados candidatos, no conocimiento definitivo \\
  ...
  \end{tabularx}
  \vfill
  \begin{block}{Lectura del proceso}
...
\begin{frame}{CRISP-DM: organizar el proyecto completo}
  \centering
  \scriptsize\textbf{CRISP-DM}: \textit{Cross-Industry Standard Process for Data Mining}\\
  \textcolor{UAPGray}{Proceso Estándar Intersectorial para Minería de Datos}
  \vspace{0.15em}
  \begin{columns}[c]
...
% 24
\begin{frame}{Etapas de CRISP-DM explicadas}
  \scriptsize
  \begin{tabularx}{\textwidth}{>{\bfseries}p{0.19\textwidth} X X}
    \toprule
    Etapa & Qué se hace & Qué debe quedar \\
    \midrule
    Comprensión del problema & Acordar necesidad, usuarios, acciones, éxito y restricciones & Ficha y criterios de aceptación \\
    Comprensión de datos & Inventariar significado, cobertura, calidad y riesgos & Diccionario, perfil y límites conocidos \\
    ...
    Evaluación & Contrastar desempeño, utilidad, equidad y riesgo & Continuar, modificar o detener \\
    Despliegue o entrega & Integrar el resultado y observar sus efectos & Producto, monitoreo, responsables y retirada \\
    \bottomrule
  \end{tabularx}
  \vfill
  \begin{block}{Proceso iterativo}
